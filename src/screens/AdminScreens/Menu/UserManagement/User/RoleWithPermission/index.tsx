...
import { useListAdminRoleMutation } from '../../../../../../injectEndpoints/userTypeEndpoints';
...
const RoleCard = ({ item, index, navigation }: RoleProps) => {
  return (
    <TouchableAtom
      style={styles.card}
      onPress={() => {
        navigation.navigate(screensName.RoleWithPermissionDetails, {
          roleId: item.id,
        });
      }}
    >
...
const RoleWithPermission = (props: Props) => {
  ...
  const renderListBedDetails = ({ item, index }: any) => (
    <RoleCard item={item} index={index} navigation={navigation} />
  );
