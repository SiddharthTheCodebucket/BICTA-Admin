...
import { BASE_URL_COPY_LINK } from '../../../../../../config';
...
const FeedbackTopic = (props: Props) => {
  const { navigation } = props;

  const { crediantialData } = useAppSelector(state => state.Auth);

  const [listFeedbackTopicApi] = useListFeedbackTopicMutation();
  const [updateFeedbackTopicApi] = useUpdateFeedbackTopicMutation();
  const [deleteFeedbackTopicApi] = useDeleteFeedbackTopicMutation();
  ...
  useLayoutEffect(() => {
    Header.setNavigation(navigation, 'Feedback Topic Details');
    ...
    const updateStatus = (id: any) => {
      ...
      updateFeedbackTopicApi(formData)
        ...
        .then((res: any) => {
          ...
          setInitialCall(false);
          listFeedbackTopic(1, true, search);
        })
        .catch((err: any) => {
          setInitialCall(false);
          Toast.show({
    ...
    return (
      <ViewAtom style={styles.card}>
        <View style={[styles.rowBetween, { marginBottom: vh(10) }]}>
          ...
          <View style={{ flexDirection: 'row', gap: vw(15) }}>
            <TouchableAtom
              ...
              onPress={() => {
                const url = `${BASE_URL_COPY_LINK}open-feedback/${item.feedbackTopicIdentifier}`;
            ...
            <TouchableAtom
              ...
              onPress={() => {
                navigation.navigate(screensName.AddFeedbackTopic, {
                  item: item,
                  onDone: () => listFeedbackTopic(1, true, search),
                });
              }}
            >
              <ImageAtom
        ...
        <View style={{ flex: 1 }}>
          ...
          <TextAtom style={styles.value}>{item.feedbackTopic ?? '-'}</TextAtom>
        </View>
        <View style={{ flex: 1 }}>
    ...
    <SafeAreaView edges={['bottom']} style={styles.container}>
      ...
      <FlatList
        ...
        onEndReached={() => {
          setPagination(true);
          nextPageAvailable
            ? listFeedbackTopic(page + 1, false, search)
            : setPagination(false);
        }}
