...
const ListPermissionCard = ({
  ...
  return (
    <TouchableAtom
      style={styles.card}
      onPress={() => {
        navigation.navigate(screensName.HouseKeepingFeedbackResponseDetails, {
          data: item,
        });
      }}
    >
...
const HouseKeepingFeedbackResponse = (props: Props) => {
  ...
  useLayoutEffect(() => {
    Header.setNavigation(navigation, 'House Keeping Feedback');
  ...
  const getFeedbackCategory = () => {
    ...
    commonDropdownApi(params)
      ...
      .then((res: any) => {
        ...
        const defaultCat = res.data.find((i: any) => i.id === 2);
        if (defaultCat) {
...
export default HouseKeepingFeedbackResponse;
