...
const ListPermissionCard = ({
  ...
  return (
    <ViewAtom style={styles.card}>
      <View style={[styles.rowBetween, { marginBottom: vh(10) }]}>
        ...
        <View style={styles.actionRow}>
          <TouchableAtom
            ...
            onPress={() => {
              navigation.navigate(screensName.AddParticulars, {
                item: item,
                onDone: onRefresh,
              });
            }}
      ...
      <View style={{ flex: 1 }}>
        <TextAtom style={styles.label}>{'Particulars Name'}</TextAtom>
        <TextAtom numberOfLines={0} style={styles.value}>
          {item.particulars ?? '-'}
        </TextAtom>
      </View>
      <View style={{ flex: 1 }}>
        <TextAtom style={styles.label}>{'Rate'}</TextAtom>
        <TextAtom numberOfLines={0} style={styles.value}>
          {item.rate ?? '-'}
        </TextAtom>
      </View>
...
const Particulars = (props: Props) => {
  const { navigation } = props;
  const { crediantialData } = useAppSelector(state => state.Auth);

  const [listReportFacultyApi] = useBudgetListParticularsMutation();
  const [deleteResponseApi] = useBudgetDeleteParticularsMutation();
  ...
  useLayoutEffect(() => {
    Header.setNavigation(navigation, 'Particulars List');
