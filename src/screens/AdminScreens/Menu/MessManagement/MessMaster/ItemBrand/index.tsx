    ...
    <View style={styles.card}>
      <View style={[styles.rowBetween, { marginBottom: vh(10) }]}>
        ...
        <View style={styles.actionRow}>
          <TouchableAtom
            ...
            onPress={() =>
              navigation.navigate(screensName.AddItemBrand, {
                item,
                onDone: onRefresh,
              })
            }
      ...
      <View style={styles.rowBetween}>
        <View style={styles.flex1}>
          <TextAtom style={styles.label}>{'Brand Name'}</TextAtom>
...
const ItemBrand = (props: Props) => {
  const { navigation } = props;

  const { crediantialData } = useAppSelector(state => state.Auth);
  const [firstTimeLoad, setFirstTimeLoad] = useState(true);
  const [listApi] = useMessManagementListItemBrandMutation();
  const [deleteApi] = useMessManagementDeleteItemBrandMutation();
  ...
  useLayoutEffect(() => {
    Header.setNavigation(navigation, 'Item Brand');
