...
import { useVisitorListQrCodeMutation } from '../../../../../../injectEndpoints/visitorManagementEndpoints';
...
const VendorCard: React.FC<VendorCardProps> = ({ item, index, navigation }) => {
  return (
    <ViewAtom style={styles.card}>
      ...
      <View style={{ flex: 1 }}>
        <TextAtom style={styles.label}>Visitor QR Name</TextAtom>
...
const QRListDetails = (props: Props) => {
  const { navigation } = props;
  const { crediantialData } = useAppSelector(state => state.Auth);

  const [listVendorApi] = useVisitorListQrCodeMutation();
  ...
  useLayoutEffect(() => {
    Header.setNavigation(navigation, 'Visitor QR List Details');
