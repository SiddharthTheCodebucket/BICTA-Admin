...
import { useReportListTimeTableMutation } from '../../../../../../injectEndpoints/reportEndpoints';
...
const ListPermissionCard = ({ item, index }: ListPermissionProps) => {
  return (
    <ViewAtom style={styles.card}>
      ...
      <View style={styles.rowBetween}>
        <View style={{ flex: 1 }}>
          <TextAtom style={styles.label}>{'Faculty'}</TextAtom>
          <TextAtom style={styles.value}>{item.selectFaculty ?? '-'}</TextAtom>
        </View>
        <View style={{ flex: 1, alignItems: 'flex-end' }}>
          <TextAtom style={styles.labelRight}>{'F-Id'}</TextAtom>
          <TextAtom style={styles.valueRight}>
            {item.selectFacultyId ?? '-'}
          </TextAtom>
        </View>
      </View>
      <View style={styles.rowBetween}>
        ...
        <View style={{ flex: 1, alignItems: 'flex-end' }}>
          <TextAtom style={styles.labelRight}>{'VC'}</TextAtom>
          <TextAtom style={styles.valueRight}>
            {item.isCombinedClass ?? '-'}
          </TextAtom>
        </View>
      </View>
    </ViewAtom>
...
const FacultyUpcomingClassTimeTable = (props: Props) => {
  const { navigation } = props;
  const item = props.route.params.item;

  const [commonDropdownApi] = useCommonDropdownListMutation();
  const [listReportFacultyApi] = useReportListTimeTableMutation();
  ...
  const baseFilters = [['selectFacultyId', '=', item?.facultyId]];
  ...
  useLayoutEffect(() => {
    Header.setNavigation(navigation, 'Upcoming Class Details');
    ...
    useCallback(() => {
      if (firstTimeLoad && !centerSerach?.name && search === '') {
        ...
        listReportFaculty(1, true, search, [
          ['date', '=', moment(item?.classDate).format('YYYY-MM-DD')],
        ]);
        getTrainingList();
      }
  ...
  const listReportFaculty = (
    ...
    const params: any = {
      ...
      filters: [...baseFilters, ...filtersArray],
      pageNo: pageNumber,
  ...
  const buildFilters = () => {
    ...
    if (selectedTraining?.id) {
      filters.push(['trainingNameId', '=', selectedTraining.id]);
    }
  ...
  const hitFilterApi = ({
    ...
    if (training?.id) {
      filters.push(['trainingNameId', '=', training.id]);
    }

    if (batch?.id) {
      filters.push(['batchNoId', '=', batch.id]);
    }
...
export default FacultyUpcomingClassTimeTable;
