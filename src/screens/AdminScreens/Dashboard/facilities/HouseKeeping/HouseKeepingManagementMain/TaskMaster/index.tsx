...
const ListPermissionCard = ({
  ...
  return (
    <TouchableAtom
      style={styles.card}
      onPress={() => {
        navigation.navigate(screensName.TaskMasterDetails, {
          data: item,
        });
      }}
    >
      <View style={[styles.rowBetween, { marginBottom: vh(10) }]}>
        ...
        <View style={styles.actionRow}>
          <TouchableAtom
            ...
            onPress={() => {
              navigation.navigate(screensName.AddTaskMaster, {
                item: item,
                onDone: onRefresh,
              });
            }}
      ...
      <View style={{ flex: 1 }}>
        <TextAtom style={styles.label}>{'Location Campus'}</TextAtom>
        <TextAtom numberOfLines={0} style={styles.value}>
          {item.locationCampus ?? '-'}
        </TextAtom>
      </View>
      <View style={{ flex: 1 }}>
        <TextAtom style={styles.label}>{'Building Type'}</TextAtom>
        <TextAtom numberOfLines={0} style={styles.value}>
          {item.buildingType ?? '-'}
        </TextAtom>
      </View>
      <View style={{ flex: 1 }}>
        <TextAtom style={styles.label}>{'Building Or Area'}</TextAtom>
        <TextAtom numberOfLines={0} style={styles.value}>
          {item.buildingOrArea ?? '-'}
        </TextAtom>
      </View>
...
const TaskMaster = (props: Props) => {
  const { navigation } = props;
  const { crediantialData } = useAppSelector(state => state.Auth);

  const [listReportFacultyApi] = useHouseKeepingListTaskMasterMutation();
  const [deleteResponseApi] = useHouseKeepingDeleteTaskMasterMutation();
  ...
  useLayoutEffect(() => {
    Header.setNavigation(navigation, 'Task Master Details');
