...
const FeedbackCategory = (props: Props) => {
  const { navigation } = props;

  const { crediantialData } = useAppSelector(state => state.Auth);

  const [listFeedbackCategoryApi] = useListFeedbackCategoryMutation();
  const [updateFeedbackCategoryApi] = useUpdateFeedbackCategoryMutation();
  const [deleteFeedbackCategoryApi] = useDeleteFeedbackCategoryMutation();
  ...
  useLayoutEffect(() => {
    Header.setNavigation(navigation, 'Feedback Category Details');
  ...
  const listFeedbackCategory = (
    ...
    listFeedbackCategoryApi(params)
      .unwrap()
      .then((res: any) => {
  ...
  const handleSearch = useCallback(
    debounce((text: string) => {
      listFeedbackCategory(1, true, text);
    }, 500),
    [],
  );
  ...
  const SubLocationDetailsCard = ({ item, index, navigation }: any) => {
    ...
    const updateStatus = (id: any) => {
      ...
      updateFeedbackCategoryApi(formData)
        .unwrap()
        .then((res: any) => {
          Toast.show({
            type: 'success',
          ...
          setInitialCall(false);
          listFeedbackCategory(1, true, search);
        })
        .catch((err: any) => {
          setInitialCall(false);
          Toast.show({
    ...
    const deleteFeedbackCategory = (id: any) => {
      ...
      deleteFeedbackCategoryApi(params)
        .unwrap()
        .then((res: any) => {
          Toast.show({
            type: 'success',
    ...
    return (
      <ViewAtom style={styles.card}>
        <View style={[styles.rowBetween, { marginBottom: vh(10) }]}>
          ...
          <View style={{ flexDirection: 'row', gap: vw(15) }}>
            <TouchableAtom
              ...
              onPress={() => {
                navigation.navigate(screensName.AddFeedbackCategory, {
                  item: item,
                  onDone: () => listFeedbackCategory(1, true, search),
                });
              }}
            >
              <ImageAtom
        ...
        <View style={{ flex: 1 }}>
          <TextAtom style={styles.label}>{'Feedback Category'}</TextAtom>
    ...
    <SafeAreaView edges={['bottom']} style={styles.container}>
      ...
      <FlatList
        ...
        onEndReached={() => {
          setPagination(true);
          nextPageAvailable
            ? listFeedbackCategory(page + 1, false, search)
            : setPagination(false);
        }}
