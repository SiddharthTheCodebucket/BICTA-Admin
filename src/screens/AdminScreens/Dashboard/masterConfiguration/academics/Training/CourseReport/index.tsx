...
} from '../../../../../../../injectEndpoints/courseReportMangementEndpoints';
        ...
        <View style={styles.actionRow}>
          ...
          >
            <ImageAtom
              ...
              style={[styles.iconSmall, { tintColor: colors.green }]}
            />
          </TouchableAtom>
        </View>
      </View>
...
const CourseReport = (props: Props) => {
  const { navigation } = props;
  const { crediantialData } = useAppSelector(state => state.Auth);

  const [listReportFacultyApi] = useCourseReportListBasicInfoMutation();
  const [deleteResponseApi] = useCourseReportDeleteBasicInfoMutation();
  const [downloadReportApi] = useCourseReportDownloadMutation();
  ...
  useLayoutEffect(() => {
    Header.setNavigation(navigation, 'Course Report Details');
  ...
  const deleteResponse = (id: any) => {
    setInitialCall(true);
    deleteResponseApi({ courseReportId: id })
      .unwrap()
      .then((res: any) => {
  ...
  const downloadReport = (id: any) => {
    setInitialCall(true);
    downloadReportApi({ courseReportId: id })
      .unwrap()
      .then((res: any) => {
        Toast.show({ type: 'success', text2: res.data.message });
        setInitialCall(false);
        setFirstTimeLoad(true);
        if (res.data.mergedFileUrl) {
          downloadAndOpenFile(res.data.mergedFileUrl);
        }
      })
      .catch((err: any) => {
        setInitialCall(false);
